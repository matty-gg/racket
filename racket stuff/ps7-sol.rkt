...
(define (right-truncatable-prime? p)
  (if (= p 1) #f
      (if (prime? p)
          (cond ((< p 10) #t)
                (else (right-truncatable-prime? (floor (/ p (expt 10 (- (count p 0) 1) ) ))))
          ) #f)))
;6bii
(define (nth-right-trunc-prime p)
  (find (lambda (x) x) right-truncatable-prime?  p))

;6ci
(define (two-sided-prime? n)
  (if (and (equal? (left-truncatable-prime? n) #t)(equal? (right-truncatable-prime? n) #t))
      #t
      #f))
;6cii
(define (nth-two-sided-prime p)
  (find (lambda (x) x) two-sided-prime?  p))
